package com.alternative_twilight.at_mod.config;
 ...
 *
 * <p>配置分端规则（见 docs/架构设计.md §4.4）：
 * 战斗与 Boss 数值由 common 消费；Boss 音乐与极光渲染配置仅由客户端消费。
 * 每个键必须注明单位、默认值与有效范围（comment）。
 */
public final class Config {
    ...
    static {
        ...
        LICH_MUSIC_ENABLED = BUILDER
                .comment("Enable background music when a Lich is nearby").define("enabled", true);
        LICH_MUSIC_DETECTION_RADIUS = BUILDER
                .comment("Player-to-Lich distance at which the music starts").defineInRange("detectionRadius", 200.0, 4.0, 10000.0);
        LICH_MUSIC_VOLUME = BUILDER
                .comment("Volume multiplier for the Lich background music").defineInRange("volume", 1.0, 0.0, 1.0);
        BUILDER.pop();
